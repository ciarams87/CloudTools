...
class Connections:
    ...
    def ec2_connection(self):
        # Creates and return an EC2 Connection object
        conn = boto.ec2.connect_to_region(self.region)
        return conn

    def s3_connection(self):
        # Creates and return an S3 connection object
        conn = S3Connection(aws_access_key_id, aws_secret_access_key)
        return conn

    def cw_connection(self):
        # Creates and return a CloudWatch connection object
        conn = boto.ec2.cloudwatch.connect_to_region(self.region)
        return conn

    def as_connection(self):
        # Creates and return an Auto Scaling connection object
        conn = boto.ec2.autoscale.connect_to_region(self.region)
        return conn

    def ct_connection(self):
        # Creates and return a CloudTrail connection object
